...
class GoodGenerator(nn.Module):
    def __init__(self, dim=DIM, output_dim=OUTPUT_DIM):
        ...
        self.ln1 = nn.Linear(self.dim, 4*4*8*self.dim)
    ...
    def generate_data(self, N, base_dist='fixed_iso_gauss'):
        ...
        if base_dist == 'fixed_iso_gauss':
            seed = torch.randn(N, 128) 
            if next(self.parameters()).is_cuda:
                seed = seed.cuda()

            seeds = torch.chunk(seed, 10, dim=0)
            all_gen = []
            for sd in seeds:
                all_gen.append(self.forward(sd).data)
            return torch.cat(all_gen, dim=0).reshape(-1, 3, 64, 64), seed
...
class netd_dcgan_par(nn.Module):
    ...
    def __init__(self, d=128, num_gpus=4):
        ...
        self.disc = nn.Sequential(
            nn.Conv2d(3, d, (nI, nJ), stride=2, padding=(round(nI/2), round(nJ/2))),
            nn.BatchNorm2d(d),
            #nn.Tanh(),
            nn.LeakyReLU(),
            ...
            nn.Conv2d(8*d, 1, (nI+1, nJ), stride=1, padding=(0, 0)),
    ...
    def forward(self, inp):
        ...
        inp = inp.view(-1, 3, 320, 456)
        x = nn.parallel.data_parallel(self.disc, inp, [1,2,3])#range(self.num_gpus))
...
class netg_dcgan_par(nn.Module):
    def __init__(self, K, d=128, num_gpus=4):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(K, 8*d, (nI + 1, nJ), stride=1, padding=(0, 0)),
            nn.BatchNorm2d(8*d),
            #nn.Tanh(),
            nn.LeakyReLU(),
            ...
            nn.ConvTranspose2d(d, 3, (nI, nJ + 1), stride=2, padding=(round(nI/2) - 7, round(nJ/2) - 3)),
            nn.Tanh(),
    ...
    def forward(self, input):
        ...
        x = nn.parallel.data_parallel(self.decoder, input, [1,2,3]) #range(self.num_gpus))
        return x
